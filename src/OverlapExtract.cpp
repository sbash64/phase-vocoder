...
OverlapExtract<T>::OverlapExtract(int N, int hop)
    : cached(sizeNarrow<T>(N)), head{0}, hop{hop}, N{N} {}

template <typename T>
void add_(const_signal_type<T> x, int N, signal_index_type<T> &head,
    buffer_type<T> &cached, const_signal_type<T> &signal) {
    auto toFill = std::min(N - head, size(x));
    std::copy(begin(x), begin(x) + toFill, begin(cached) + head);
